<!-- Footer-osa -->
<footer class="py-20 sm:py-32 bg-black border-t border-solid border-violet-950 flex flex-col gap-4 sm:gap-8 justify-center items-center">
    <!-- Yhteydenottokutsu -->
    <p class="px-4 py-2 bg-white text-slate-950 font-medium">
        Connect with me;
    </p>
    <!-- Yhteystiedot -->
    <div class="flex flex-col gap-4 items-center justify-center">
        <!-- Sähköpostiosoite -->
        <p><b class="pr-2">Email</b> cauco_dev@gmail.com</p>
        <!-- GitHub-linkki -->
        <p>
            <b class="pr-2">GitHub</b>
            <a href="https://github.com/CAUCORASEKO" target="_blank" class="text-violet-400">
                CAUCO
                <!-- Ikoni, joka osoittaa, että linkki avautuu uuteen välilehteen -->
                <sup>
                    <span class="text-xs scale-75 pl-0.5">
                        <i class="fa-solid fa-arrow-up-right-from-square text-xs scale-[75%]"></i>
                    </span>
                </sup>
            </a>
        </p>
        <!-- LinkedIn-linkki -->
        <p>
            <b class="pr-2">LinkedIn</b>
            <a href="https://www.linkedin.com/in/multimedia3d/" target="_blank" class="text-violet-400">
                CAUCO
                <!-- Ikoni, joka osoittaa, että linkki avautuu uuteen välilehteen -->
                <sup>
                    <span class="text-xs scale-75 pl-0.5">
                        <i class="fa-solid fa-arrow-up-right-from-square text-xs scale-[75%]"></i>
                    </span>
                </sup>
            </a>
        </p>
    </div>
</footer>
